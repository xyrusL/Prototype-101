...
def logging_function(number, mode, landmark_list, point_history_list, num_per_class):
    ...
    for i in range(num_per_class):
        ...
        time.sleep(0.4)  # 400 milliseconds
...
def main():
    ...
    hands = mp_hands.Hands(
        ...
        max_num_hands=3,  # Limit to 3 hands
    ...
    while True:
        ...
        right_hand_count = 0  # Counter for right hands

        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                if handedness.classification[0].label == 'Right':  # Only process right hands
                    right_hand_count += 1
                    if right_hand_count > 3:  # Limit to 3 right hands
                        break

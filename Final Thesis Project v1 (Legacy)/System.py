...
def move_servo_to_center(wrist_x, frame_width, previous_x, smooth_factor=0.1):
    wrist_x = previous_x * (1 - smooth_factor) + wrist_x * smooth_factor
    servo_angle = map_range(wrist_x, 0, frame_width, -20, 200)
...
# Main function
def main():
    global speech_recognition_active, stop_speech_recognition, recognized_speech_text, speech_thread, automatic_listening, confidence, handedness, brect, help_show, voice_version
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                if handedness.classification[0].label == 'Right':
                    ...
                    confidence = calculate_confidence(pre_processed_landmark_list, right_hand_classifier) * 100
                else:
                    hand_sign_id = left_hand_classifier(pre_processed_landmark_list)
                    current_label = left_hand_labels[hand_sign_id]
                    confidence = calculate_confidence(pre_processed_landmark_list, left_hand_classifier) * 100

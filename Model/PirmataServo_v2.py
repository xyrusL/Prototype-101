...
# Configuration for mediapipe and pyfirmata
...
# ESP camera URL
url = 'http://192.168.254.137/1024x768.jpg'

while True:
    # Fetching image from ESP camera
    imgResp = urllib.request.urlopen(url)
    imgNp = np.array(bytearray(imgResp.read()), dtype=np.uint8)
    img = cv2.imdecode(imgNp, -1)
    img = cv2.resize(img, (ws, hs))  # Resize to match the original OpenCV capture size
    img = cv2.flip(img, 1)
